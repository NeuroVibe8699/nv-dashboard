...
def login():
    ...
    # Master Admin Access (Bypasses DB if needed)
    if u == "admin@nvpredictive.com" and p == "admin123":
        return jsonify({"status": "success", "role": "admin", "redirect": "/admin.html"}), 200
        
    # Check for Clients in DB
    try:
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute("SELECT * FROM users WHERE username=%s AND password_hash=%s", (u, p))
        user = cur.fetchone()
        cur.close()
        conn.close()
        if user:
            return jsonify({"status": "success", "role": "client", "redirect": "/dashboard.html"}), 200
    except: pass
    
    return jsonify({"status": "error", "message": "Invalid Credentials"}), 401

# --- 2. INVENTORY (Bulk Import with 868/915 Logic) ---
...
def bulk_import():
    try:
        ...
        for _, row in df.iterrows():
            ...
            # Frequency Logic: Odd model digit = 868, Even = 915
...
def list_users():
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT id, username, company_name FROM users WHERE role='client'")
    return jsonify(cur.fetchall())
